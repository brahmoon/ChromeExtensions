...
const WORKSPACE_REORDER_MESSAGE = 'WorkspaceReorderItems';
...
// background のマスターキャッシュから受け取った最新タブスナップショット。
// refreshTabs はまずこのキャッシュを使い、null の場合のみ chrome.tabs.query にフォールバックする。
let cachedTabListSnapshot = null;
...
let tabPanelSuppressDragDropClickUntil = 0;
...
function getWorkspaceImagePreviewElement() {
  return document.getElementById(WORKSPACE_IMAGE_PREVIEW_ID);
}

function closeWorkspaceImagePreview() {
  const preview = getWorkspaceImagePreviewElement();
  if (!preview || preview.hidden) {
    return;
  }
  ...
  const image = document.getElementById(WORKSPACE_IMAGE_PREVIEW_IMAGE_ID);
  if (image) {
...
function openWorkspaceImagePreview(item) {
  if (!item?.imageUrl) {
    return;
  }
  ...
  image.alt = item.pageTitle || '';
  preview.hidden = false;
  preview.setAttribute('aria-hidden', 'false');
}

function setupWorkspaceImagePreviewControls() {
  const preview = getWorkspaceImagePreviewElement();
  if (!preview) {
    return;
  }

  const closeButton = document.getElementById(WORKSPACE_IMAGE_PREVIEW_CLOSE_ID);
  if (closeButton) {
...
function scheduleWorkspaceTextClampMeasurement() {
  if (workspaceTextMeasureRaf !== null) {
    cancelAnimationFrame(workspaceTextMeasureRaf);
  }

  workspaceTextMeasureRaf = requestAnimationFrame(() => {
    workspaceTextMeasureRaf = null;
    updateWorkspaceTextClampState();
  });
}
...
function updateWorkspaceTextClampState() {
  ...
  const textElements = grid.querySelectorAll('.workspace-card__text');
  textElements.forEach((textEl) => {
    const expandButton = textEl.parentElement?.querySelector('.workspace-card__text-expand');
  ...
  if (item.type === 'image' && item.imageUrl) {
    ...
    img.addEventListener('click', (event) => {
      ...
      clickTimer = setTimeout(() => {
        clickTimer = null;
        card.classList.toggle('workspace-card--image-expanded');
      }, 220);
    });
    img.addEventListener('dblclick', (event) => {
      ...
      card.classList.add('workspace-card--image-expanded');
      openWorkspaceImagePreview(item);
    });
...
async function initializePanelSyncSnapshotState() {
  try {
    ...
    // シーケンス・タイムスタンプを初期化。
    // 拡張リロード後は background 側のシーケンスも 0 からリスタートするため、
    // ここで保持している値は「現在ストレージにある最新スナップショット」の番号として扱い、
    // 次の storage.onChanged で同番号以上が来た時だけ再描画する。
    ...
    // ストレージに保存されているタブスナップショットをメモリキャッシュに取り込む。
    // これにより、パネル起動直後の refreshTabs が chrome.tabs.query を叩く前に
    // background マスターキャッシュの内容を使えるようになる。
    if (Array.isArray(tabListEntity.tabs)) {
      cachedTabListSnapshot = tabListEntity.tabs;
    }
  } catch (error) {
...
function setupTabGroupDragAndDrop(root) {
  ...
  root.addEventListener('click', (event) => {
    if (Date.now() >= tabPanelSuppressDragDropClickUntil) {
      return;
    }
    const groupHeader = event.target instanceof Element ? event.target.closest('.group-header') : null;
    if (!(groupHeader instanceof HTMLElement)) {
      return;
    }
    event.preventDefault();
  ...
  root.addEventListener('dragover', (event) => {
    ...
    const evtTarget = event.target instanceof Element ? event.target : null;
    if (!evtTarget) {
    ...
    // ── ドロップ先とインジケーター位置の解決 ─────────────────────────────
    //
    // 通常表示: #tab-list > .group-item
    // 全画面表示: #expanded-view > .window-column > .window-column__list > .group-item
    //
    // 余白ドロップの問題:
    //   .window-column__list に flex:1 を付与してコンテナ高いっぱいに伸ばしているが、
    //   念のため .window-column 自体にヒットした場合も内部の __list にフォールバックする。
    const LIST_SELECTOR = '#tab-list, .window-column__list';

    let resolvedTarget = null;   // インジケーターを付けるアイテム
    let dropBefore = true;
    ...
    if (hoverGroup instanceof HTMLElement && hoverGroup !== dragging.source) {
      ...
      if (event.clientY < rect.top + rect.height / 2) {
        // 上半分 → このアイテムの直前
        resolvedTarget = hoverGroup;
        dropBefore = true;
      } else {
        // 下半分 → 次の .group-item が存在すればそこの drop-before に昇格
        const next = hoverGroup.nextElementSibling;
        if (
          next instanceof HTMLElement &&
          next.classList.contains('group-item') &&
          next !== dragging.source
        ) {
          resolvedTarget = next;
          dropBefore = true;
        } else {
      ...
      // ── ケース②: リストコンテナの余白にカーソルがある ────────────────
      // .window-column__list にヒットするのが通常だが、
      // CSS の都合で .window-column 自体にヒットする場合は内部の __list を探す。
      let listContainer = evtTarget.closest(LIST_SELECTOR);
      if (!(listContainer instanceof HTMLElement)) {
        const windowColumn = evtTarget.closest('.window-column');
        if (windowColumn instanceof HTMLElement) {
          listContainer = windowColumn.querySelector('.window-column__list');
        }
      }
      ...
      if (event.clientY < firstTop) {
        resolvedTarget = first;
        dropBefore = true;
      } else {
        resolvedTarget = allItems[allItems.length - 1];
        dropBefore = false;
      }
    }
    ...
    const targetMeta = groupMetadataMap.get(resolvedTarget);
    ...
    clearGroupDropIndicators(root);
    resolvedTarget.classList.add(dropBefore ? 'group-item--drop-before' : 'group-item--drop-after');
    pendingDropTarget = { targetGroupItem: resolvedTarget, dropBefore };
    dragging.moved = true;
  });
  ...
  root.addEventListener('drop', async (event) => {
    ...
    if (pendingNewWindowGroupDrop) {
      ...
      if (Number.isFinite(sourceGroupId) && sourceGroupId >= 0) {
        ...
        try {
          ...
          if (dragging.moved) {
            tabPanelSuppressGroupHeaderClickUntil = Date.now() + 300;
            tabPanelSuppressDragDropClickUntil = tabPanelSuppressGroupHeaderClickUntil;
          }
          await refreshTabs();
        } catch (err) {
    ...
    try {
      if (isCrossWindowGroupDrop) {
        ...
        const sourceGroupInfo = dragging.metadata?.groupInfo;
        if (!Number.isFinite(sourceGroupId) || sourceGroupId < 0) {
          throw new Error('Invalid source group id in group cross-window drop');
        }
...
async function moveTabToWindow(tabId, targetWindowId, targetIndex) {
  if (!Number.isFinite(tabId) || !Number.isFinite(targetWindowId) || !Number.isFinite(targetIndex)) {
    throw new Error('Invalid arguments for moveTabToWindow');
  }
...
async function moveGroupToWindow(sourceGroupId, targetWindowId, targetIndex, sourceGroupInfo) {
  if (!Number.isFinite(sourceGroupId) || sourceGroupId < 0 || !Number.isFinite(targetWindowId) || !Number.isFinite(targetIndex)) {
    throw new Error('Invalid arguments for moveGroupToWindow');
  }
  ...
  for (let i = 1; i < tabs.length; i++) {
    await chrome.tabs.move(tabs[i].id, { windowId: targetWindowId, index: targetIndex + i });
  }
...
async function setupTabDragAndDrop(root, { expandedMode = false } = {}) {
  ...
  root.addEventListener('click', (event) => {
    if (!expandedMode || Date.now() >= tabPanelSuppressDragDropClickUntil) {
      return;
    }
  ...
  root.addEventListener('dragover', (event) => {
    ...
    // ── ドロップ先とインジケーター位置の解決 ─────────────────────────────
    //
    // 通常表示・全画面表示で共通のリストセレクター。
    // .window-column__list を含めることで expanded view の余白にも対応する。
    const LIST_SELECTOR = '#tab-list, .group-tab-list, .window-column__list';
    ...
    if (hoverTab instanceof HTMLElement && hoverTab !== dragging.source) {
      ...
      if (event.clientY < rect.top + rect.height / 2) {
        // 上半分 → このアイテムの直前
        resolvedTarget = hoverTab;
        dropBefore = true;
      } else {
        // 下半分 → 次の .tab-item が存在すればそこの drop-before に昇格
        // （A下辺と B上辺が近接して2本見えるのを防ぐ統一ルール）
        const next = hoverTab.nextElementSibling;
        if (
          next instanceof HTMLElement &&
          next.classList.contains('tab-item') &&
          next !== dragging.source
        ) {
          resolvedTarget = next;
          dropBefore = true;
    ...
    clearTabDropIndicators(root);
    resolvedTarget.classList.add(dropBefore ? 'tab-item--drop-before' : 'tab-item--drop-after');
    pendingTabDropTarget = { targetTabItem: resolvedTarget, dropBefore };
  });
  ...
  root.addEventListener('dragend', () => {
    ...
    if (movedByDragAndDrop) {
      ...
      tabPanelSuppressDragDropClickUntil = tabPanelSuppressTabItemClickUntil;
    }
    clearTabDropIndicators(root);
    pendingTabDropTarget = null;
    pendingNewWindowDrop = false;
...
async function refreshTabs() {
  ...
  if (expandedViewEnabled) {
    ...
    for (const win of orderedWindows) {
      ...
      headerLabel.className = 'window-column__header-label';
      if (win?.id === windowId) {
        headerLabel.textContent = '現在のウィンドウ';
      } else {
        headerLabel.textContent = Number.isFinite(win?.id) ? `ウィンドウ ${win.id}` : '別ウィンドウ';
      }
      header.appendChild(headerLabel);

      // ウィンドウを閉じる × ボタン（windowId が有効な場合のみ表示）
      if (Number.isFinite(win?.id)) {
        const closeBtn = document.createElement('button');
        closeBtn.className = 'window-column__close-btn';
        closeBtn.type = 'button';
        closeBtn.title = 'ウィンドウを閉じる';
        closeBtn.setAttribute('aria-label', 'ウィンドウを閉じる');
        closeBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12z"/></svg>';
  ...
  // background のマスターキャッシュ（cachedTabListSnapshot）が存在する場合はそれを優先使用する。
  // これにより拡張リロード直後や再注入直後でも chrome.tabs.query を省略できる。
  // windowId フィルタが必要な場合はキャッシュをウィンドウで絞り込む。
  // キャッシュが null（まだ受信していない）場合のみ chrome.tabs.query にフォールバックする。
  if (cachedTabListSnapshot !== null) {
    tabs = Number.isFinite(windowId)
      ? cachedTabListSnapshot.filter((t) => t.windowId === windowId)
      : cachedTabListSnapshot.slice();
  } else {
    try {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setupWorkspaceImagePreviewControls();
  setupSearchControls();
  setupCloseButton();
  ...
  if (tabListSyncChange) {
    ...
    if (nextValue && typeof nextValue === 'object') {
      ...
      if (shouldApplyBySequence || shouldApplyByTimestamp) {
        ...
        // background から受け取った最新スナップショットをキャッシュに反映する。
        // refreshTabs はこのキャッシュを優先して使用するため、
        // chrome.tabs.query を省略した高速・確実なリフレッシュが行われる。
        if (Array.isArray(nextValue.tabs)) {
          cachedTabListSnapshot = nextValue.tabs;
        }
        refreshTabs();
      }
    }
