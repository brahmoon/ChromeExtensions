...
const WORKSPACE_CAPTURE_TTL_MS    = 3000;

// ── Workspace: pendingCapture（補完情報のみ・TTL付き） ───────────
...
// ── Workspace: Shadow DOM トースト ────────────────────────────────
let _wsToastContainer = null;
let _wsToastTimer     = null;

function showWorkspaceToast(text) {
  if (!_wsToastContainer) {
    _wsToastContainer = document.createElement('div');
    _wsToastContainer.setAttribute(EXTENSION_ELEMENT_ATTRIBUTE, '');
    _wsToastContainer.style.cssText = `
      position: fixed;
      bottom: 28px;
      left: 50%;
    ...
    const shadow = _wsToastContainer.attachShadow({ mode: 'open' });
    ...
    (document.body || document.documentElement).appendChild(_wsToastContainer);
  }

  const toastEl = _wsToastContainer.shadowRoot.querySelector('.toast');
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (message.type === 'openPanel') {
    ...
    closePanel();
  } else if (message.type === WORKSPACE_SHOW_TOAST_MSG) {
    showWorkspaceToast(message.text ?? 'ワークスペースに保存しました');
  }
});

// WorkspaceFlushPendingCapture は sendResponse が必要なため別途登録
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  const isFresh =
    capture !== null &&
    Date.now() - capture.capturedAt <= WORKSPACE_CAPTURE_TTL_MS;
