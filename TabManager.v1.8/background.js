...
const WORKSPACE_SAVE_VIDEO_MENU_ID  = 'workspace-save-video';
...
// ── YouTube URL から videoId を抽出するヘルパー ───────────────────
// background.js 内で動画保存メニューのハンドラから呼び出す。
// panel.js の extractYoutubeVideoId と同じロジックを独立実装
// （background は panel の関数にアクセスできないため）。
  ...
  try {
    ...
    if (host === 'youtube.com') {
      if (u.pathname.startsWith('/shorts/') || u.pathname.startsWith('/embed/')) {
        return u.pathname.split('/')[2]?.slice(0, 11) || null;
      }
...
function registerWorkspaceContextMenus() {
  chrome.contextMenus.removeAll(() => {
    ...
    // YouTube サムネイル（image）・サムネイルリンク（link）・動画ページ（page）の
    // 3コンテキストで表示。documentUrlPatterns で YouTube ドメインに限定。
    chrome.contextMenus.create({
      id:                 WORKSPACE_SAVE_VIDEO_MENU_ID,
      title:              'ワークスペースに動画を保存',
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (
    ...
    menuId !== WORKSPACE_SAVE_VIDEO_MENU_ID
  ) {
    return;
  }
  if (!tab?.id) return;
  ...
  // ── YouTube 動画保存 ──────────────────────────────────────────────
  // サムネイル(image)・サムネイルリンク(link)・動画ページ(page) の
  // 3コンテキストすべてで pageUrl / linkUrl から動画 URL を決定する。
  if (menuId === WORKSPACE_SAVE_VIDEO_MENU_ID) {
    // 優先順: linkUrl（リンク右クリック）> pageUrl（ページ右クリック / 画像右クリック）
    const rawUrl = info.linkUrl || info.pageUrl || tab.url || '';

    // YouTube URL から動画 URL を正規化（/watch?v= 形式に統一）
    const videoId = extractYoutubeVideoIdFromUrl(rawUrl);
    if (!videoId) return; // YouTube URL でなければ何もしない
    ...
    } catch { /* chrome:// 等では失敗 → tab.title をそのまま使用 */ }

    // タイトルが「- YouTube」サフィックスを含む場合は除去してクリーンに保存
    pageTitle = pageTitle.replace(/\s*[-–]\s*YouTube\s*$/, '').trim() || pageTitle;

    const item = {
      id:        crypto.randomUUID(),
      type:      'text',       // YouTube URL はテキスト型として保存（既存の埋め込み表示を活用）
      text:      videoUrl,
      imageUrl:  null,
      pageUrl:   videoUrl,
      pageTitle: pageTitle,
    ...
    try {
      ...
      console.error('Workspace save video: IndexedDB write failed', error);
