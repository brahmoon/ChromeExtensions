...
async function moveTabGroupByDragAndDrop(sourceMeta, targetMeta, dropBefore) {
  // chrome.tabGroups.move() が使えない環境はフォールバックなし（MV3 Chrome 89+ 必須）
  ...
  // ソースとターゲット両グループの現在情報を取得
  let sourceGroup = null;
  let targetTabs = [];
  try {
    [sourceGroup, targetTabs] = await Promise.all([
      chrome.tabGroups.get(sourceGroupId),
      chrome.tabs.query({ groupId: targetGroupId }),
    ]);
  } catch (error) {
    console.error('Failed to query group info for drag and drop:', error);
    return;
  }
  ...
  targetTabs = Array.isArray(targetTabs)
    ? targetTabs.filter((tab) => Number.isFinite(tab?.index))
    : [];
  ...
  // chrome.tabGroups.move() の index はタブストリップ上のタブインデックスを指定する。
  // dropBefore=true  → ターゲットグループの先頭タブの位置に移動（直前に配置）
  // dropBefore=false → ターゲットグループの末尾タブの次の位置に移動（直後に配置）
...
//
// 設計方針：
//   dragover は「どこにドロップしようとしているか」を pendingDropTarget に記録するだけ。
//   DOM の insertBefore は一切行わない（視覚フィードバックはCSSインジケーターのみ）。
//   drop は pendingDropTarget の記録を読んで chrome.tabGroups.move() を呼ぶ。
//
//   これにより「dragover でDOMが動いたせいで drop 時の当たり判定がずれる」問題を
//   構造ごと排除する。
...
function setupTabGroupDragAndDrop(root) {
  ...
  // ドロップ先を記録する状態。dragover が毎フレーム上書きし、drop が読む。
  ...
  root.addEventListener('dragover', (event) => {
    ...
    if (!(targetGroup instanceof HTMLElement) || targetGroup === dragging.source) {
      // ドロップ先候補がなければインジケーターをクリアして記録もリセット
  ...
  root.addEventListener('drop', async (event) => {
    ...
    try {
      await moveTabGroupByDragAndDrop(dragging.metadata, targetMeta, pending.dropBefore);
      if (dragging.moved) {
