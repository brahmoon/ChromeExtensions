...
async function getAllWorkspaceItems({ groupId } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req   = groupId ? store.index('groupId').getAll(groupId) : store.getAll();
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  // Step 1: info（Chrome直接提供）でメイン情報を確定
  ...
  // Step 2: content.js から pageTitle を補完（失敗しても続行）
  // capture は pageTitle 取得のみが実質用途。
  // TTL検証は content.js 側で実施済み（古い capture は null で返る）。
  let pageTitle = tab.title || '';
  try {
    const res = await chrome.tabs.sendMessage(tab.id, { type: WORKSPACE_FLUSH_CAPTURE_MSG });
    if (res?.capture?.pageTitle) pageTitle = res.capture.pageTitle;
  } catch {
    // chrome:// 等では失敗する → tab.title をそのまま使用
  }
...
async function sendPanelCommand(tabId, type) {
  ...
  try {
    ...
    // content script が未注入（拡張リロード直後など）の場合のみ再注入を試みる
    ...
    console.debug('Panel command: content script unavailable, attempting re-injection:', message);
  }

  // content script を動的注入する（拡張リロード後の未注入状態を回復）
  try {
    ...
    // chrome:// や file:// など注入不可なページは静かに失敗させる
    console.debug('Panel command: content script injection failed:', injectError?.message);
    return false;
  }
  ...
  try {
    ...
    console.debug('Panel command failed after re-injection:', retryError?.message);
    return false;
  }

  // 再注入直後はタブスナップショットが古い可能性があるため
  // マスターキャッシュを最新状態で上書きし、panel.js に強制プッシュする
  persistTabListSyncEntity('reinjection').catch((error) => {
    console.debug('Panel command: failed to push tab list after re-injection:', error);
  });
...
function getComparableTabUrlForGrouping(tab) {
  if (!tab) {
    return '';
  }
...
function hasTabOrderChanged(currentTabs, sortedTabs) {
  if (!Array.isArray(currentTabs) || !Array.isArray(sortedTabs)) {
    return false;
  }

  if (currentTabs.length !== sortedTabs.length) {
    return true;
  }

  for (let index = 0; index < currentTabs.length; index += 1) {
    if (currentTabs[index]?.id !== sortedTabs[index]?.id) {
      return true;
    }
  }
...
async function sortTabsInGroupByUrlPath(windowId, groupId) {
  ...
  if (!Array.isArray(groupTabs) || groupTabs.length < 2) {
    return false;
  }

  const sortableTabs = groupTabs
    .filter((tab) => tab && Number.isFinite(tab.id))
    .map((tab) => ({ ...tab, url: getComparableTabUrlForGrouping(tab) }));

  if (sortableTabs.length < 2) {
    return false;
  }

  const sortedTabs = sortTabsByPathHierarchy(sortableTabs);
  if (!hasTabOrderChanged(sortableTabs, sortedTabs)) {
    return false;
  }

  const sortedIds = sortedTabs.map((tab) => tab.id);
  const firstIndex = Math.min(...sortableTabs.map((tab) => tab.index));

  try {
    await chrome.tabs.move(sortedIds, { index: firstIndex });
    return true;
  } catch (error) {
    console.debug('Failed to sort tabs in group by URL path:', error);
    return false;
  }
}

async function sortAllTabGroupsInWindow(windowId) {
  if (!Number.isFinite(windowId)) {
    return false;
  }
  ...
  for (const tab of tabs) {
    if (!tab || !Number.isFinite(tab.groupId) || tab.groupId < 0 || tab.pinned) {
      continue;
    }
  ...
  for (const groupId of groupIds) {
    const changed = await sortTabsInGroupByUrlPath(windowId, groupId);
...
async function autoGroupTabByDomainCore(tab, { requireActiveContext } = {}) {
  ...
  try {
    ...
    const sortedGroups = await sortAllTabGroupsInWindow(tab.windowId);

    if (normalized || sortedGroups || !tabAlreadyInDestination || migrateTabIds.length > 0 || destination.kind === 'domain-from-misc') {
...
async function groupTabsByDomain({ scope, windowId }) {
  ...
  for (const [entryWindowId, entry] of perWindow.entries()) {
    ...
    const sortedGroups = await sortAllTabGroupsInWindow(entryWindowId);
    if (sortedGroups) {
      hasGroupingChanges = true;
    }
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // ── Workspace: panel.js からのデータ取得・削除 ─────────────────
