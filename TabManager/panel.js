...
const PREVIEW_COLUMN_ID = 'preview-column';
const PREVIEW_WRAPPER_ID = 'preview-wrapper';
...
const PREVIEW_IFRAME_SANDBOX = 'allow-same-origin';
...
function getPreviewColumn() {
  return document.getElementById(PREVIEW_COLUMN_ID);
}

function getPreviewWrapper() {
  return document.getElementById(PREVIEW_WRAPPER_ID);
}
...
function renderPreviewPlaceholder(message, { loading = false } = {}) {
  ...
  if (loading) {
    placeholder.classList.add('preview-placeholder--loading');
    if (message) {
...
function renderPreviewImage(preview, tab) {
  ...
  iframe.srcdoc = `<!DOCTYPE html><html><head><style>body{margin:0;background:#111;display:flex;align-items:center;justify-content:center;}img{width:100%;height:100%;object-fit:cover;}</style></head><body><img src="${escapeHtmlAttribute(image)}" alt="${escapeHtmlAttribute(title)}"></body></html>`;
...
function updatePreviewVisibility() {
  ...
  column.classList.toggle('is-visible', previewEnabled);
  if (!previewEnabled) {
...
async function initializePreviewState() {
  try {
    const stored = await chrome.storage.local.get({
      [PREVIEW_ENABLED_STORAGE_KEY]: false,
    ...
    previewCache = {};
    applyPreviewSettings(false, { skipRefresh: true });
  }
