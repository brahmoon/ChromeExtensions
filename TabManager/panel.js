...
const HISTORY_BACK_BUTTON_ID = 'history-back-btn';
const HISTORY_FORWARD_BUTTON_ID = 'history-forward-btn';
...
const suppressedHistoryUpdateTabIds = new Set();
...
function getHistoryBackButton() {
  return document.getElementById(HISTORY_BACK_BUTTON_ID);
}

function getHistoryForwardButton() {
  return document.getElementById(HISTORY_FORWARD_BUTTON_ID);
}
...
function updateHistoryNavigationState() {
  const backButton = getHistoryBackButton();
  const forwardButton = getHistoryForwardButton();

  const entries = getHistoryEntriesForOpenTabs();
  const currentIndex = entries.findIndex(
    (entry) => entry && entry.tabId === lastKnownActiveTabId
  );
  ...
  if (forwardButton) {
    forwardButton.disabled = currentIndex <= 0;
  }
}
...
async function activateTabByIdWithoutHistoryUpdate(tabId) {
  if (!Number.isFinite(tabId) || tabId < 0) {
    return;
  }
  ...
  if (chrome?.tabs?.get) {
    try {
      tabDetails = await chrome.tabs.get(normalizedTabId);
    } catch (error) {
      console.debug('Failed to retrieve tab details for navigation:', error);
    }
  }

  suppressedHistoryUpdateTabIds.add(normalizedTabId);

  try {
    await chrome.tabs.update(normalizedTabId, { active: true });
    if (
      tabDetails &&
      Number.isFinite(tabDetails.windowId) &&
      chrome?.windows?.update
    ) {
      await chrome.windows.update(tabDetails.windowId, { focused: true });
    }
  } catch (error) {
    suppressedHistoryUpdateTabIds.delete(normalizedTabId);
    console.error('Failed to activate tab from history navigation:', error);
  }
}

async function handleHistoryNavigation(step) {
  const entries = getHistoryEntriesForOpenTabs();
  if (entries.length === 0) {
    return;
  }
  ...
  if (!targetEntry || !Number.isFinite(targetEntry.tabId)) {
    return;
  }

  await activateTabByIdWithoutHistoryUpdate(targetEntry.tabId);
}
...
function setupHistoryControls() {
  ...
  if (backButton) {
    backButton.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      handleHistoryNavigation(1).catch(() => {});
    });
  }
...
function handleTabActivated(activeInfo) {
  ...
  if (suppressedHistoryUpdateTabIds.has(normalizedTabId)) {
    suppressedHistoryUpdateTabIds.delete(normalizedTabId);
    updateHistoryNavigationState();
    return;
  }
