...
const DOMAIN_GROUP_OTHER_TITLE = 'その他';
const DOMAIN_GROUP_COLOR_PALETTE = ['blue', 'green', 'purple', 'cyan', 'yellow', 'orange', 'pink'];
const DOMAIN_GROUP_OTHER_COLOR = 'grey';
...
function extractDomainFromUrl(urlString) {
  ...
  try {
    ...
    const hostname = typeof url.hostname === 'string' ? url.hostname.trim() : '';
...
function pickColorForDomain(domain) {
  if (typeof domain !== 'string' || domain.length === 0) {
    return DOMAIN_GROUP_OTHER_COLOR;
  }

  if (!Array.isArray(DOMAIN_GROUP_COLOR_PALETTE) || DOMAIN_GROUP_COLOR_PALETTE.length === 0) {
    return 'blue';
  }

  let hash = 0;
  for (let index = 0; index < domain.length; index += 1) {
    hash = (hash * 33 + domain.charCodeAt(index)) >>> 0;
  }

  const color = DOMAIN_GROUP_COLOR_PALETTE[hash % DOMAIN_GROUP_COLOR_PALETTE.length];
  return color || DOMAIN_GROUP_COLOR_PALETTE[0];
}

async function ungroupTabsIfPossible(tabIds) {
  if (!Array.isArray(tabIds) || tabIds.length === 0) {
    return;
  }
  ...
  try {
    if (chrome.tabs.ungroup.length > 1) {
    ...
    console.debug('Failed to ungroup tabs before regrouping:', error);
  }
}

async function createTabGroup(tabIds, updateProperties = {}) {
  const normalizedIds = dedupeTabIds(tabIds);
  if (normalizedIds.length === 0) {
    return null;
  }

  try {
    const groupId = await chrome.tabs.group({ tabIds: normalizedIds });
    if (typeof groupId !== 'number') {
      return null;
    }

    if (chrome?.tabGroups?.update && updateProperties && typeof updateProperties === 'object') {
      const { title, color } = updateProperties;
      const payload = {};
      ...
      if (Object.keys(payload).length > 0) {
        try {
          await chrome.tabGroups.update(groupId, payload);
        } catch (error) {
          console.debug('Failed to update tab group metadata:', error);
        }
      }
    }
...
async function groupTabsByDomainWithinWindow(windowId, tabs) {
  ...
  for (const tab of tabs) {
    ...
    if (domain) {
      let bucket = domainMap.get(domain);
      if (!bucket) {
        bucket = [];
  ...
  const tabIdsForUngroup = dedupeTabIds(tabs.map((tab) => tab && tab.id));
  if (tabIdsForUngroup.length > 0) {
    await ungroupTabsIfPossible(tabIdsForUngroup);
  }
  ...
  for (const [domain, domainTabs] of sortedDomains) {
    if (!Array.isArray(domainTabs) || domainTabs.length <= 1) {
      remainder.push(...(domainTabs || []));
      continue;
    }

    const tabIds = dedupeTabIds(domainTabs.map((tab) => tab && tab.id));
    if (tabIds.length <= 1) {
      remainder.push(...domainTabs);
      continue;
    }

    const groupId = await createTabGroup(tabIds, {
      title: domain,
  ...
  const remainderIds = dedupeTabIds(remainder.map((tab) => tab && tab.id));
  if (remainderIds.length > 0) {
    const otherGroupId = await createTabGroup(remainderIds, {
      title: DOMAIN_GROUP_OTHER_TITLE,
      color: DOMAIN_GROUP_OTHER_COLOR,
    });
...
async function resolveTabsForGrouping(includeAllWindows, windowId) {
  ...
  if (targetWindowId == null && chrome?.windows?.getLastFocused) {
    try {
      if (chrome.windows.getLastFocused.length > 0) {
        targetWindowId = await new Promise((resolve) => {
          chrome.windows.getLastFocused({ populate: false }, (windowInfo) => {
            if (chrome.runtime.lastError) {
              resolve(null);
              return;
            }
            resolve(windowInfo && typeof windowInfo.id === 'number' ? windowInfo.id : null);
          });
        });
      } else {
  ...
  if (targetWindowId == null) {
    throw new Error('Failed to determine target window for grouping.');
  }

  return chrome.tabs.query({ windowId: targetWindowId });
}

async function groupTabsByDomain({ includeAllWindows = false, windowId = null } = {}) {
  const tabs = await resolveTabsForGrouping(includeAllWindows, windowId);
  if (!Array.isArray(tabs) || tabs.length === 0) {
    return { grouped: 0 };
  }
  ...
  for (const tab of tabs) {
    ...
    let bucket = tabsByWindow.get(tab.windowId);
    if (!bucket) {
      bucket = [];
      tabsByWindow.set(tab.windowId, bucket);
    }
    bucket.push(tab);
  }
  ...
  for (const [windowKey, windowTabs] of tabsByWindow.entries()) {
    try {
      totalGroups += await groupTabsByDomainWithinWindow(windowKey, windowTabs);
    } catch (error) {
      console.error('Failed to group tabs in window', windowKey, error);
    }
  }

  return { grouped: totalGroups };
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === GROUP_TABS_BY_DOMAIN_MESSAGE) {
    (async () => {
      try {
        const includeAllWindows = message.scope === 'all';
