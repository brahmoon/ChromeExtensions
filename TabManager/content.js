...
if (!window.__tabManagerMessageHandler) {
  const messageHandler = (event) => {
    ...
    if (event.data?.type === 'TabManagerClosePanel') {
      closePanel();
      chrome.runtime.sendMessage({ type: 'TabManagerPanelClosedByUser' }).catch(() => {});
    }
  };
...
async function initializePanelState() {
  try {
    const response = await chrome.runtime.sendMessage({ type: 'TabManagerQueryPanelState' });
    if (response?.open) {
      openPanel();
    }
  } catch (error) {
    // The background script may not be reachable on restricted pages.
    console.debug('TabManager: unable to query initial panel state', error);
  }
}
