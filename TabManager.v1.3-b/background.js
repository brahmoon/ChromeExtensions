...
// タスク4: ウィンドウ単位の自動グループ化デバウンス管理
const autoGroupDebounceTimers = new Map();
const AUTO_GROUP_DEBOUNCE_MS = 300;

/**
 * ウィンドウIDをキーとしてデバウンスし、autoGroupTabByDomain を呼び出すラッパー。
 * 同一ウィンドウへの連続イベント（セッション復元・大量 onUpdated 等）でも
 * 最後の呼び出しのみが実行されるため、Chrome API のレート制限やレースコンディションを防ぐ。
 */
function scheduleAutoGroupTabByDomain(tab, options = {}) {
  if (!autoDomainGroupingEnabled || !tab || !Number.isFinite(tab.windowId)) {
    return;
  }
  ...
  const timerId = setTimeout(() => {
    ...
    autoGroupTabByDomain(tab, options).catch(() => {});
  }, AUTO_GROUP_DEBOUNCE_MS);

  autoGroupDebounceTimers.set(windowId, { timerId, tab, options });
}
...
chrome.tabs.onCreated.addListener((tab) => {
  ...
  // タスク4: scheduleAutoGroupTabByDomain によるデバウンスで連続タブ作成時の過剰呼び出しを抑制。
  scheduleAutoGroupTabByDomain(tab);
...
chrome.tabs.onAttached.addListener(async (tabId, attachInfo) => {
  ...
  // タスク3-a: ウィンドウ間ドラッグ移動後、アタッチ先でグループ化判定を実行。
  // onMoved は同一ウィンドウ内の並べ替えのみで、ウィンドウ跨ぎ移動は onDetached → onAttached で通知される。
  if (autoDomainGroupingEnabled) {
    try {
      const tab = await chrome.tabs.get(tabId);
      scheduleAutoGroupTabByDomain(tab);
    } catch (error) {
      console.debug('Failed to get tab for auto-grouping after attach:', error);
    }
  }
});
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (tab) {
    // タスク1: requireActiveContext を廃止し、URL 変化時は非アクティブタブ・サブウィンドウも対象とする。
    // タスク3-b: status === 'complete' でも判定を走らせ、SPA遷移（changeInfo.url が来ない場合）を補完。
    // タスク4: scheduleAutoGroupTabByDomain によるウィンドウ単位デバウンスで連続呼び出しを抑制。
    if (changeInfo.url) {
      scheduleAutoGroupTabByDomain(tab);
...
async function autoGroupTabByDomain(tab, { requireActiveContext } = {}) {
  ...
  // タスク1: requireActiveContext ゲートを廃止。
  // 旧実装では tab.active && lastFocusedWindow === tab.windowId の両条件を満たさないと
  // スキップしていたため、サブウィンドウでフォーカスが外れた瞬間に処理が取りこぼされていた。
  // URL 変化を検知した時点でウィンドウ・タブのアクティブ状態に関わらずグループ化判定を実行する。
  // （requireActiveContext 引数は互換性のために残すが、判定には使用しない）
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (autoGroupChange) {
    ...
    if (!wasEnabled && autoDomainGroupingEnabled) {
      // タスク2: 設定 ON 時の初回グループ化を全ウィンドウ対象に変更。
      // 旧実装は GROUP_SCOPE_CURRENT + lastFocusedWindow のみだったため、
      // サブウィンドウが初回グループ化から除外されていた。
