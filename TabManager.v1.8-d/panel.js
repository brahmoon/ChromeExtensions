...
//
//   4. 移動不要判定は点比較ではなく範囲チェック（sourceStart ～ sourceEnd）。
//      グループ内への挿入は順序変化なしのためスキップする。
//
...
async function moveTabGroupByDragAndDrop(sourceMeta, targetMeta, dropBefore) {
  ...
  // 移動不要判定：ソースグループ内（sourceStart ～ sourceEnd）への挿入は順序変化なしのためスキップ
  if (targetIndex >= sourceStart && targetIndex <= sourceEnd) {
    return;
  }
