...
function getCurrentChannelId() {
  const host = window.location.hostname; // 例: "dashboard.twitch.tv"
  const pathParts = window.location.pathname.split('/').filter(Boolean);

  // dashboard.twitch.tv/u/{channelId}/... の形式に対応
  if (host.startsWith('dashboard.') && pathParts[0] === 'u') {
    return pathParts[1] ? pathParts[1].toLowerCase() : '';
  }

  // popout チャット: twitch.tv/popout/{channelId}/...
  if (pathParts[0] === 'popout') {
    return pathParts[1] ? pathParts[1].toLowerCase() : '';
  }

  // 通常の配信ページ: twitch.tv/{channelId}
