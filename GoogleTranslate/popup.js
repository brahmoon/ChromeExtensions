...
function createNewlineVisualization(token) {
  const wrapper = document.createElement('span');
  wrapper.className = 'newline-token';
  const label = token === '\r\n' ? 'CRLF' : token === '\r' ? 'CR' : 'LF';
  wrapper.dataset.label = `↵ ${label}`;
  wrapper.title = `改行 (${label})`;
  wrapper.setAttribute('role', 'img');
  wrapper.setAttribute('aria-label', `改行 (${label})`);
...
function renderTextWithNewlineIndicators(element, value) {
  ...
  for (const part of parts) {
    ...
    if (part === '\r\n' || part === '\n' || part === '\r') {
      hasNewlines = true;
      element.appendChild(createNewlineVisualization(part));
    } else {
